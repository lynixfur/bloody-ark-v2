...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  if(current_page < 69) {
    next_page = `https://mesa-ark.com/api/ark/tribe_rankings?page=${parseInt(current_page as any) + 1}&search=${search}`;
  }

  if(current_page > 0) {
    prev_page = `https://mesa-ark.com/api/ark/tribe_rankings?page=${parseInt(current_page as any) - 1}&search=${search}`;
  }
