...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  typeof value === 'bigint'
    ...
    if(current_page < Math.round(pages / 20)) {
      next_page = `https://bloody.gg/api/player_rankings?page=${parseInt(current_page as CurrentPage) + 1}&search=${search}`;
    }
  
    if(current_page > 0) {
      prev_page = `https://bloody.gg/api/player_rankings?page=${parseInt(current_page as CurrentPage) - 1}&search=${search}`;
    }
