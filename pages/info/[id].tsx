...
export async function getServerSideProps({ params }: any) {
  const id = params.id;
  const res = await fetch(`${env.DOMAIN}/api/info/${id}`);
  const list_res = await fetch(`${env.DOMAIN}/api/info/page_list`);
    ...
    <div className="block md:flex w-full min-w-full">
        <ul className="flex-col py-2 space-y-1 text-gray-300 px-2 mt-10 w-full md:w-64 md:border-r border-gray-700">
          ...
          {page_list.map((page_link: any) => (
                      <a key={page_link.str_id} href={"/info/" + page_link?.str_id} className="transition-colors duration-300 flex items-center w-full py-2.5 px-3 mt-2 rounded text-forward hover:bg-bgray-overlay">
